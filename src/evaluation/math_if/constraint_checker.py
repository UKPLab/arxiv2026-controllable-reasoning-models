...
from .constraint_util import LANGUAGE_CODES, generate_keywords, count_words
...
# LEVEL_1_NUM_WORDS_LOWER_LIMIT = 100
# LEVEL_1_NUM_WORDS_UPPER_LIMIT = 500

NUM_WORDS_LOWER_LIMIT = 128
NUM_WORDS_UPPER_LIMIT = 1024
...
class ResponseLanguageChecker(Constraint):
    ...
    def build_description(self, *, language=None):
        ...
        self._description_pattern = (
            "Your answer should be in {language} language, no other language is allowed. "
        )
...
# class NumberOfSentences(Constraint):
...
#     def check_following(self, value):
#         """Check if the number of sentences follows the Constraint.

#         Args:
#           value: A string representing the response.

#         Returns:
#           True if the response follows the Constraint.

#         Raise:
#             ValueError if the string in `Constraint_args` is not in
#             [`less_than`, `at_least`].
#         """
#         num_sentences = Constraints_util.count_sentences(value)
...
# class PlaceholderChecker(Constraint):
...
#     def check_following(self, value):
#         """Check if the number of placeholders follows the Constraint.
...
class BulletListChecker(Constraint):
...
# class ConstrainedResponseChecker(Constraint):
...
# class ConstrainedStartChecker(Constraint):
...
#           contained in `Constraint_args`; otherwise, False.
#         """
...
class HighlightSectionChecker(Constraint):
...
class SectionChecker(Constraint):
    """Checks the sections."""
...
# class ParagraphChecker(Constraint):
...
# class PostscriptChecker(Constraint):
...
#           the keyword containing in the `Constraint_args`; otherwise False.
#         """
#         value = value.lower()
...
# class RephraseChecker(Constraint):
#     """Checks the rephrase."""
...
#     def check_following(self, value):
#         r"""Checks if the rephrasing follows the Constraint.

#         Args:
#           value: A string representing the response, which is expected to rephras
#             the string of `Constraint_args`.

#         Returns:
#           True if `value` and `Constraint_args` only differ by the words/sentences
...
class KeywordChecker(Constraint):
...
class KeywordFrequencyChecker(Constraint):
...
# class ParagraphFirstWordCheck(Constraint):
#     """Check the paragraph and the first word of the nth paragraph."""

#     def build_description(
#         self, num_paragraphs=None, nth_paragraph=None, first_word=None
#     ):
#         r"""Build the Constraint description.
...
# class KeySentenceChecker(Constraint):
...
#         count = 0
#         sentences = Constraints_util.split_into_sentences(value)
...
class ForbiddenWords(Constraint):
...
# class RephraseParagraph(Constraint):
...
# class TwoResponsesChecker(Constraint):
...
class RepeatPromptThenAnswer(Constraint):
...
# class LetterFrequencyChecker(Constraint):
...
class LowercaseLettersEnglishChecker(Constraint):
...
class CapitalWordFrequencyChecker(Constraint):
...
class QuotationChecker(Constraint):
